...
export default class CairoNamespaceNode extends BaseNode {
  /**
  ...
  constructor(name: string, startLine: number, parents: BaseNode[]) {
    super(name, startLine, parents, EntitiesType.nameSpace);
  }
  ...
  static isTextLineThisNode(
    ...
    if (textLine.trim().startsWith("namespace")) {
      return true;
    }
  ...
  static createNode(
    ...
    const regex = /namespace\s+(\w+)/;
    const match = regex.exec(textLine);
    if (match) {
      return new CairoNamespaceNode(match[1], lineNumber, parents);
    }
