...
suite("Traverse Test Suite", async () => {
  test("Test traverse reecursively", async () => {
    // We test import statements and get its object
    const cwd = vscode.workspace.workspaceFolders![0].uri.fsPath;
    const cairoParser = new CairoParser();

    const contractCode = "from lib_test import ERC20";
  ...
  test("Test get local and custom filepath", async () => {
    ...
    assert.equal(
      cairoFileFinder.getFilePath(importPath),
      path.join(cwd, "test_utils/lib.cairo")
    );
    ...
    assert.equal(
      cairoFileFinder.getFilePath(importPath3),
      path.join(cwd, "test_utils/this_cairo/hello.cairo")
    );
